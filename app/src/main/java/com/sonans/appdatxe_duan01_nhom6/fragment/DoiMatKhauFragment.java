...
public class DoiMatKhauFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        SharedPreferences sharedPreferences = getActivity().getSharedPreferences("MaKHang", MODE_PRIVATE);
        String makhachhang = sharedPreferences.getString("maKH", "");
      ...
      btnChangePassword.setOnClickListener(new View.OnClickListener() {
          ...
          public void onClick(View view) {
              ...
              collectionReference.whereEqualTo("tenDangNhap", makhachhang).get().addOnCompleteListener(task -> {
                  if (task.isSuccessful()){
                      for (QueryDocumentSnapshot documentSnapshot: task.getResult()){
                          KhachHang khachHang = documentSnapshot.toObject(KhachHang.class);
                          if (khachHang.getMatKhau().equals(etOldPassword.getText().toString())){
                              collectionReference.document(documentSnapshot.getId()).update("matKhau", etNewPassword.getText().toString()).addOnSuccessListener(new OnSuccessListener<Void>() {
                                  @Override
